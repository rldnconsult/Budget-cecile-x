...
module.exports = async function handler(req, res) {
  ...
  try {
    const { imageDataUrl, ocrText = '', hint = '', mode = 'multi' } = JSON.parse((await readBody(req)) || '{}');
    if (!imageDataUrl && !ocrText) return send(res, 400, { ok:false, error:'Image requise.' });
    const receiptSchema = {
      type:'object', additionalProperties:false,
      properties:{
        merchant:{type:['string','null']}, amount:{type:['number','null']},
        ...
        confidence:{type:'number'}, comment:{type:['string','null']}, warnings:{type:'array',items:{type:'string'}},
      ...
      required:['merchant','amount','date','category','confidence','comment','warnings','crop']
    };
    ...
    const content = [{ type:'input_text', text:`Analyse une photo de justificatif pour une étudiante. Il peut y avoir un seul ticket OU plusieurs tickets visibles sur la même photo. Renvoie un objet par ticket reconnu indépendamment. Pour chaque ticket, extrais le montant TTC total, la date d'achat, l'enseigne, la catégorie probable et une zone de recadrage normalisée crop (x,y,width,height entre 0 et 1) autour du ticket, avec petite marge. Si la photo contient un seul ticket, renvoie un seul objet. Si un ticket est incertain, renvoie-le quand même avec confidence faible et warnings. Catégories: food=alimentation/courses, restaurants=restaurant/snack/café, outings=sorties/loisirs, gifts=cadeaux, transport, health, hygiene, study, clothes, subscriptions, bank, equipment, home, other. Indice utilisateur: ${hint || 'aucun'}. Texte OCR éventuel: ${ocrText || 'aucun'}.` }];
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body:JSON.stringify({ model: process.env.OPENAI_RECEIPT_MODEL || 'gpt-5-mini', input:[{role:'user',content}], max_output_tokens: 900, text:{format:{type:'json_schema',name:'receipt_batch',strict:true,schema}} })
    });
    clearTimeout(timer);
    ...
    if (!receipts.length) receipts = [{ merchant:null, amount:null, date:null, category:'other', confidence:0, comment:null, warnings:['Aucun ticket détecté avec certitude.'], crop:null }];
